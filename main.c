...
void setLogicalPixel(Color *pixelColors, int gridX, int gridY, int screenWidth, int drawSize, Color color) {
...
void explodeNapalm(Color *pixelColors, Pixel *pixels, int gridWidth, int gridHeight, int screenWidth, int drawSize, int x, int y) {
    ...
    for (int c = 0; c < clusterCount; c++) {
        ...
        for (int yy = centerY - radius; yy <= centerY + radius; yy++) {
            ...
            for (int xx = centerX - radius; xx <= centerX + radius; xx++) {
                        ...
                        if (!blocked && targetX >= 0 && targetX < gridWidth && targetY >= 0 && targetY < gridHeight) {
                            ...
                            if (target->draw == 'n') {
                                *target = *p;
                                ...
                                setLogicalPixel(pixelColors, targetX, targetY, screenWidth, drawSize, target->color);
                            }
                        }
                    }
                ...
                setLogicalPixel(pixelColors, xx, yy, screenWidth, drawSize, p->color);
            }
        }
    }
...
int main() {
    ...
    const int reactionRange = 50 / drawSize;
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            ...
            if (mousePos.y >= 10 && mousePos.y <= 40) {
                ...
                if (mousePos.x <= screenWidth - 10 && mousePos.x >= screenWidth - 10 - 30 && drawAmount > 1) { 
                    drawAmount /= 2; 
                } else if (mousePos.x <= screenWidth - 30 - 10 - 10 - textSize2 - 10 && mousePos.x >= screenWidth - 30 - 10 - 10 - textSize2 - 10 - 30 && drawAmount < 64) { 
                    drawAmount *= 2; 
                }
            }
                ...
                for (int step = 0; step <= steps; step++) {
                    ...
                    for (int gy = centerY - half; gy < centerY - half + drawAmount; gy++) { // Dont really understand these for loops tbh
                        for (int gx = centerX - half; gx < centerX - half + drawAmount; gx++) {
                            ...
                            setLogicalPixel(pixelColors, gx, gy, screenWidth, drawSize, p->color);
                        }
                    }
                }
                prevMouse = mousePos;
        ...
        pixelCount = 0;
        for (int i = gridWidth * gridHeight - 1; i >= 0; i--) {
            if (pixels[i].draw == 'y') {
                pixelCount++;
                Pixel temp = pixels[i];
                int col = i % gridWidth;
                ...
                switch (pixels[i].type) {
                    case 's': // Sand rules: If can go down, go down, if can go left or right, choose random, if can only go one way do that, else stay still
                        if (pixels[i].y < gridHeight - 1 && pixels[i + gridWidth].type != 'S' && pixels[i + gridWidth].type != 'i' && pixels[i + gridWidth].type != 'o') {
                            ...
                            if (pixels[i + gridWidth].draw == 'n') {
                                ...
                                setLogicalPixel(pixelColors, pixels[i + gridWidth].x, pixels[i + gridWidth].y, screenWidth, drawSize, pixels[i + gridWidth].color);
                                setLogicalPixel(pixelColors, pixels[i].x, pixels[i].y, screenWidth, drawSize, (Color){0, 0, 0, 255});
                                ...
                                setLogicalPixel(pixelColors, pixels[target].x, pixels[target].y, screenWidth, drawSize, pixels[target].color);
                                setLogicalPixel(pixelColors, pixels[i].x, pixels[i].y, screenWidth, drawSize, (Color){0, 0, 0, 255});
                                ...
                                setLogicalPixel(pixelColors, pixels[i + gridWidth - 1].x, pixels[i + gridWidth - 1].y, screenWidth, drawSize, pixels[i + gridWidth - 1].color);
                                setLogicalPixel(pixelColors, pixels[i].x, pixels[i].y, screenWidth, drawSize, (Color){0, 0, 0, 255});
                    ...
                    case 'w': // Water rules: If can go down, go down, if can go left or right choose random, Slide sideways if can't fall at all, try left or right or random,option 2 - find the furthest free pxiel next to another water pixel and fill that to make like fluid
                        ...
                        if (pixels[i].life < 0) pixels[i].life = 100;

                        if (typeNear(pixels, gridWidth, gridHeight, pixels[i].x, pixels[i].y, reactionRange, 'l')) {
                            reacted = 1;
                            pixels[i].life -= randInt(1, 2);

                            for (int n = 0; n < 4; n++) {
                                if (!waterValid[n]) continue;
                                int ni = waterNeighbors[n];

                                if (pixels[ni].type == 'w' && pixels[ni].life >= 0 && pixels[ni].life < pixels[i].life) {
                                    pixels[i].life -= 1;
                                }
                            }
                        }
                        ...
                        if (pixels[i].life <= 0) {
                            ...
                            setLogicalPixel(pixelColors, pixels[i].x, pixels[i].y, screenWidth, drawSize, pixels[i].color);
                            break;
                        }
                        ...
                        if (pixels[i].y < gridHeight - 1 && pixels[i + gridWidth].type != 'S' && pixels[i + gridWidth].type != 'i' && pixels[i + gridWidth].type != 'o') {
                            ...
                            if (pixels[i + gridWidth].draw == 'n' || fallingThroughSteam) {
                                ...
                                if (col > 0 && col < gridWidth - 1 && pixels[i - 1].draw == 'n' && pixels[i + 1].draw == 'n') {
                                    ...
                                    setLogicalPixel(pixelColors, pixels[i + 1].x, pixels[i + 1].y, screenWidth, drawSize, pixels[i + 1].color);
                                    setLogicalPixel(pixelColors, pixels[i].x, pixels[i].y, screenWidth, drawSize, (Color){0, 0, 0, 255});
                    ...
                    case 'S': // Stone rules: conduct heat from lava neighbors, melt to lava if fully heated, cool water if hot
                    {
                        ...
                        for (int n = 0; n < 4; n++) {
                            ...
                            if (pixels[ni].type == 'w') {
                                ...
                                if (pixels[ni].life <= 0) {
                                    ...
                                    setLogicalPixel(pixelColors, pixels[ni].x, pixels[ni].y, screenWidth, drawSize, pixels[ni].color);
                                }
                                ...
                                // adjacent lava reheats stone fast
                                pixels[i].heat += 1;
                        ...
                        if (pixels[i].heat <= 0) {
                            pixels[i].heat = -1; // fully cold, go dormant
                        }
                        break;
                    }
                    ...
                    case 'l': // Lava rules: react with water to cool and solidify, heat adjacent stone, flow like slow water
                    {
                        ...
                        if (typeNear(pixels, gridWidth, gridHeight, pixels[i].x, pixels[i].y, reactionRange, 'w')) {
                            reacted = 1;
                            touchedWater = 1;
                            pixels[i].heat -= randInt(1, 2);
                        }
                        ...
                        for (int n = 0; n < 4; n++) {
                            ...
                            if (pixels[neighbors[n]].type == 'i') {
                                ...
                                setLogicalPixel(pixelColors, pixels[neighbors[n]].x, pixels[neighbors[n]].y, screenWidth, drawSize, pixels[neighbors[n]].color);
                        ...
                        if (pixels[i].heat <= 0) {
                            if (touchedWater && randInt(1, 12) == 1) {
                                pixels[i].type = 'o';
                                pixels[i].heat = -1;
                                pixels[i].color = (Color){randInt(70, 110), randInt(25, 45), randInt(130, 180), 255};
                            } else {
                                pixels[i].type = 'S';
                                pixels[i].heat = randInt(40, 60); // starts warm so it can conduct through the barrier
                    ...
                    case 'p': // Napalm rules: falls like sand, explodes when lava lights it, chains into nearby napalm
                    {
                        ...
                        if (pixels[i].life > 0) {
                            ...
                            setLogicalPixel(pixelColors, pixels[i].x, pixels[i].y, screenWidth, drawSize, pixels[i].color);
                            if (pixels[i].life <= 0) {
                                explodeNapalm(pixelColors, pixels, gridWidth, gridHeight, screenWidth, drawSize, pixels[i].x, pixels[i].y);
                            }
                            break;
                        }
        ...
        SetWindowTitle(TextFormat("Sand Simulator - %i FPS - %i Particles - %i Pixels", GetFPS(), pixelCount, pixelCount * 16));
    }
